...
export function createMergedPdf(callback) {
    pdfMake.createPdf(pdfDocDefinition).getBuffer(async (buffer) => {
        ...
        for (let i = 0; i < pdfDoc.getPageCount(); i += 4) {
            ...
            const newPage = newPdfDoc.addPage([2 * page1.getWidth(), 2 * page1.getHeight()]);

            newPage.drawPage(page1, { x: 0, y: page1.getHeight() });
            if (page2) newPage.drawPage(page2, { x: page1.getWidth(), y: page1.getHeight() });
