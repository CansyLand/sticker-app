...
export async function mergePages(
	originalPdfBuffer: string | Uint8Array | ArrayBuffer
) {
	...
	for (let i = 0; i < pages.length; i += 4) {
		const [embeddedPage1, embeddedPage2, embeddedPage3, embeddedPage4] =
			await Promise.all([
		...
		if (embeddedPage2)
			newPage.drawPage(embeddedPage2, { x: pageWidth, y: pageHeight })
